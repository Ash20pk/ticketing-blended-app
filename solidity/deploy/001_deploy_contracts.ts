...
// Deploy WASM Contract
  ...
  const wasmBinaryPath = "../bin/ticketing.wasm";
...
//Deploy Solidity Contract
  console.log("Deploying TicketingApp contract...");
  const ticketValidatorAddress = checkmateValidatorAddress;

  const ticketingApp = await deploy("TicketingApp", {
    from: deployerAddress,
    args: [ticketValidatorAddress],
    log: true,
  });

  console.log(`Ticketing app contract deployed at: ${ticketingApp.address}`);
};
